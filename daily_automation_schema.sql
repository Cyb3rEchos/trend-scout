...
-- Create view for today's top 10 per category
...
-- Create view for category leaders (top 3 per category with AI recommendations)
...
AND ai_recommendation IS NOT NULL  -- Must have AI recommendations
ORDER BY category, category_rank;

-- Create view for trending analysis (compare with yesterday)
...
today AS (
    SELECT app_id, category, category_rank as curr_rank
    FROM daily_rankings 
    WHERE date = CURRENT_DATE
)
SELECT 
...
-- Create function to get daily brief data
